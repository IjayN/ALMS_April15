...
class LeaveController extends Controller
{
    ...
    */
    ...
    public function hrPendingRequests()
    {
        ...
        return response()->json([
            "error" => false,
            "message" => "HR Pending Leave Requests",
            "leaveRequests" => $pendingLeaveRequests
        ]);
    }
    ...
    public function pmPendingRequests()
    {
        ...
        $pendingLeaveRequests = LeaveApplication::where('pm', 'pending')->where('department_id', $user->department->id)->with('user', 'user.department', 'reliever')->where('releiver_approval','approved')->orderBy('id', 'desc')->get();
